...
public class AuthorApiController implements AuthorApi {
    ...
    private final AuthorApiDelegate delegate;

    public AuthorApiController(@Autowired(required = false) AuthorApiDelegate delegate) {
        this.delegate = Optional.ofNullable(delegate).orElse(new AuthorApiDelegate() {});
    }

    @Override
    public AuthorApiDelegate getDelegate() {
        return delegate;
    }
